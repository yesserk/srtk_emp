...
@section('title')
تعديل حالات الموظفين
@endsection

@section('content')
<a href="{{ route('etat.create') }}" class="btn btn-inverse-info btn-fw">اضافة الحالة</a>
...
<div class="col-lg-12 grid-margin stretch-card">
    ...
    <form method="GET" action="{{ route('etat.search_etat') }}">
    ...
    @foreach($etats as $etat)
        <tr>
            <td>{{ $etat->employee->nom }}</td>
            <td>{{ $etat->etat_emp->etat }}</td>
            <td>{{ $etat->date_etat }}</td>

           

            <th class="button-container">
    <a href="{{ route('etat.edit', $etat->id) }}" class="btn btn-success btn-sm"><i class="mdi mdi-grease-pencil"></i></a>
    <form action="{{ route('etat.destroy', $etat->id) }}" method="POST" style="display: inline;">
        @csrf
        @method('DELETE')
    ...
    @php
        $totalPages = ceil($totalEtat / $etats->perPage());
    @endphp
